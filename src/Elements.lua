--==============================================================================
-- ScorpioX Modern Elements Engine (Premium Dynamic UI)
--==============================================================================
...
-- Funzione helper per creare i container degli elementi basandosi sulle altezze del Tema
local function CreateContainer(parent, height)
	...
	Utils.Stroke(frame, Theme.Colors.Stroke, Theme.StrokeThickness or 1)
...
function Elements.Section(parent, text)
	...
	label.TextSize = Theme.SmallText or 11
...
function Elements.Paragraph(parent, title, body)
	-- Legge l'altezza direttamente dal Tema
	local frame = CreateContainer(parent, Theme.ParagraphHeight or 60)
	...
	Title.TextSize = Theme.TextSize or 13
	...
	Body.TextSize = Theme.SmallText or 11
...
function Elements.Button(parent, text, callback)
	-- Legge l'altezza dal Tema
	local frame = CreateContainer(parent, Theme.ButtonHeight or 35)
	...
	button.Text = text
	button.TextSize = Theme.TextSize or 13
	button.TextColor3 = Theme.Colors.Text
	button.Parent = frame

	button.MouseEnter:Connect(function()
		Utils.Tween(frame, {BackgroundColor3 = Theme.Colors.Tertiary}, Theme.FastTween)
	end)

	button.MouseLeave:Connect(function()
		Utils.Tween(frame, {BackgroundColor3 = Theme.Colors.Secondary}, Theme.FastTween)
	end)

	button.MouseButton1Down:Connect(function()
		Utils.Tween(frame, {BackgroundColor3 = Theme.Colors.Accent}, Theme.FastTween)
		Utils.Tween(button, {TextColor3 = Color3.new(0,0,0)}, Theme.FastTween)
	end)

	button.MouseButton1Up:Connect(function()
		Utils.Tween(frame, {BackgroundColor3 = Theme.Colors.Tertiary}, Theme.FastTween)
		Utils.Tween(button, {TextColor3 = Theme.Colors.Text}, Theme.FastTween)
	end)
...
function Elements.Toggle(parent, title, default, callback)
	-- Legge l'altezza dal Tema
	local h = Theme.ToggleHeight or 45
	local frame = CreateContainer(parent, h)
	...
	label.Text = title
	label.TextSize = Theme.TextSize or 13
	...
	Utils.Corner(bg, Theme.CircleCorner or UDim.new(1, 0))
	...
	Utils.Corner(knob, Theme.CircleCorner or UDim.new(1, 0))
	...
	local function Update(animate)
		...
		if animate then
			Utils.Tween(bg, {BackgroundColor3 = targetBg}, Theme.AnimationSpeed)
			Utils.Tween(knob, {Position = targetKnobPos, BackgroundColor3 = targetKnobColor}, Theme.AnimationSpeed)
		else
...
function Elements.Slider(parent, title, min, max, default, callback)
	-- Legge l'altezza dal Tema
	local frame = CreateContainer(parent, Theme.SliderHeight or 55)
	...
	Utils.Corner(bar, Theme.CircleCorner or UDim.new(1, 0))
	...
	Utils.Corner(fill, Theme.CircleCorner or UDim.new(1, 0))
...
function Elements.TextBox(parent, title, placeholder, callback)
	-- Legge l'altezza dal Tema
	local frame = CreateContainer(parent, Theme.TextBoxHeight or 45)
	...
	box.Font = Theme.Font
	box.TextSize = Theme.SmallText or 12
	box.ClearTextOnFocus = false
	box.Parent = frame

	Utils.Corner(box, UDim.new(0, 4))
	local stroke = Utils.Stroke(box, Theme.Colors.Stroke, Theme.StrokeThickness or 1)

	box.Focused:Connect(function()
		Utils.Tween(stroke, {Color = Theme.Colors.Accent}, Theme.FastTween)
	end)

	box.FocusLost:Connect(function(enterPressed)
		Utils.Tween(stroke, {Color = Theme.Colors.Stroke}, Theme.FastTween)
...
function Elements.Dropdown(parent, title, options, callback)
	-- Legge l'altezza di partenza dal Tema
	local baseHeight = Theme.DropdownHeight or 40
	local frame = CreateContainer(parent, baseHeight)
	frame.ClipsDescendants = true

	local button = Instance.new("TextButton")
	button.Size = UDim2.new(1, 0, 0, baseHeight)
	button.BackgroundTransparency = 1
	button.Text = "  " .. title .. "  ▼"
	button.Font = Theme.SemiBoldFont
	button.TextSize = Theme.TextSize or 13
	...
	list.Visible = false
	list.Position = UDim2.new(0, 12, 0, baseHeight + 2)
	...
	for _, option in ipairs(options) do
		...
		opt.TextSize = Theme.SmallText or 12
		opt.Parent = list

		Utils.Corner(opt, UDim.new(0, 4))

		opt.MouseEnter:Connect(function()
			Utils.Tween(opt, {TextColor3 = Theme.Colors.Accent}, Theme.FastTween)
		end)
		opt.MouseLeave:Connect(function()
			Utils.Tween(opt, {TextColor3 = Theme.Colors.TextDark}, Theme.FastTween)
		end)
		...
		opt.Activated:Connect(function()
			...
			opened = false
			list.Visible = false
			Utils.Tween(frame, {Size = UDim2.new(0.95, 0, 0, baseHeight)}, Theme.AnimationSpeed)
	...
	button.Activated:Connect(function()
		...
		if opened then
			...
			Utils.Tween(frame, {Size = UDim2.new(0.95, 0, 0, height + baseHeight + 10)}, Theme.AnimationSpeed)
		else
			list.Visible = false
			Utils.Tween(frame, {Size = UDim2.new(0.95, 0, 0, baseHeight)}, Theme.AnimationSpeed)
		end
...
function Elements.Keybind(parent, title, defaultKey, callback)
	-- Legge l'altezza dal Tema
	local frame = CreateContainer(parent, Theme.KeybindHeight or 40)
	...
	bind.TextSize = Theme.SmallText or 11
	bind.Text = defaultKey.Name
	bind.Parent = frame

	Utils.Corner(bind, UDim.new(0, 4))
	local stroke = Utils.Stroke(bind, Theme.Colors.Stroke, Theme.StrokeThickness or 1)
